...
export default class CopyUrlInPreview extends Plugin {
		...
		this.registerEvent(
			this.app.workspace.on("canvas:node-menu", (menu, node) => {
				...
				if (data.type === "link") {
					const url = data.url;
					menu.addItem((item) =>
						setMenuItem(item, "copy-to-clipboard", url).setSection(
							"canvas"
						)
					);
				}
	...
	storeLastHoveredLinkInPreview(_event: MouseEvent, link: HTMLElement): void {
		this.lastHoveredLinkTarget =
			link.getAttribute("data-href") ?? undefined;
	}
	...
	showOpenPdfMenu(event: MouseEvent | PointerEvent, el: HTMLElement): void {
		...
		if (isInCanvas) {
			const canvasCardMenu =
				activeDocument.querySelector<HTMLElement>(".menu");
			if (canvasCardMenu) {
		...
		if (this.longTapTimeoutId) {
			...
			if (event.targetTouches.length == 1) {
				this.longTapTimeoutId = window.setTimeout(
					() => void this.processLongTap.bind(this, event, img)(),
					timeouts.longTap
				);
			}
		}
	...
	async processLongTap(
		...
		const localImagePrefixUrl =
			webviewServerUrl + "/_capacitor_file_" + basePath;
		if (img.src.startsWith(localImagePrefixUrl)) {
			...
			try {
				...
				setTimeout(
					() => void adapter.remove(tempFileName),
					timeouts.deleteTempFile
				);
				new Notice(
	...
	onImageContextMenu(event: MouseEvent): void {
		...
		if (
			(!this.settings.enableDefaultOnCanvas &&
				this.app.workspace.getActiveFile()?.extension === "canvas") ||
			event.targetNode?.parentElement?.className ===
		...
		if (protocol === "app:" && relativePath) {
			...
			if (Platform.isDesktop) {
				...
				if (this.settings.revealInNavigation) {
					menu.addItem((item) =>
						setMenuItem(item, "reveal-in-navigation").onClick(
							() => {
								const file =
									this.app.vault.getFileByPath(relativePath);
								if (!file) {
									console.warn(
				...
				if (
					this.app.plugins.enabledPlugins.has("file-tree-alternative")
				) {
					menu.addItem((item) =>
	...
	onImageMouseUp(event: MouseEvent): void {
		...
		if (
			...
			const imageElement = imageElementFromMouseEvent(event);
			if (!imageElement) return;
			const imageSrc = imageElement.currentSrc;
			try {
				...
				if (relativePath) {
					...
					new Notice(`Cannot open image: ${imageSrc}`);
				}
			} catch (e) {
				console.error(e);
				new Notice(`Error opening image: ${imageSrc}`);
			}
		}
	}
}
